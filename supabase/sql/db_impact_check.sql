...
--
-- Run this through the Supabase MCP execute_sql tool after migrations that touch:
-- profiles, cpf, auth signup metadata, RLS policies, grants, triggers or onboarding.
--
-- The check uses deterministic @example.invalid users and ends with rollback, so
-- successful runs do not persist test data.
...
-- Simulate Google OAuth signup: Auth creates profiles with a null role, then the
-- authenticated user completes onboarding by updating profiles and inserting the
-- matching role profile row.
...
-- Confirm the old Google onboarding bug would still be caught: client-side
-- profiles upsert should not be a valid contract because it requires table SELECT
-- and risks broad CPF exposure if "fixed" with table-level grants.
set local role authenticated;
set local request.jwt.claim.sub = '10000000-0000-0000-0000-000000000001';
set local request.jwt.claim.role = 'authenticated';
...
-- Simulate email/password caregiver signup. Role, phone and cpf arrive in Auth
-- metadata and must be handled by the auth.users trigger.
insert into auth.users (
  id,
  aud,
  role,
...
-- Simulate email/password family signup.
insert into auth.users (
  id,
  aud,
  role,
...
-- User consent insert contract used by signup completion and pending-consent retry.
set local role authenticated;
...
-- rollback is intentional: this impact check must not persist fixtures.
rollback;

select 'db_impact_check_passed' as status;
