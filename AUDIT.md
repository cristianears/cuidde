...
- **Bloco D8 validacao:** `appointments` separada em policies por comando para participantes, preservando a permissao efetiva para familia ou cuidador vinculados; Performance Advisor deixou de listar `appointments` para policies permissive duplicadas e initplan; `pg_policies` confirmou regras preservadas; `test`, `build`, `lint` e `test:e2e` passaram.
- **Bloco D9 validacao:** `messages` consolidada em SELECT de participantes, INSERT com `sender_id` validado e permissao para cuidador ou familia assinante, e UPDATE para destinatario marcar como lida; Performance Advisor deixou de listar `messages` para policies permissive duplicadas e initplan; `pg_policies` confirmou regras; `test`, `build`, `lint` e `test:e2e` passaram.
- **Bloco D10 validacao:** `care_routines` consolidada em SELECT para familia ou cuidador participante e INSERT/UPDATE/DELETE para cuidador em atendimento `ativo`; Performance Advisor deixou de listar RLS initplan e policies permissive duplicadas; `pg_policies` confirmou uma policy por comando; `test`, `build`, `lint` e `test:e2e` passaram.
...
- **Verificacao final dos Advisors (2026-05-22):** Performance Advisor sem `auth_rls_initplan`, sem `multiple_permissive_policies` e sem `unindexed_foreign_keys`; restam `unused_index` informativos em indices recem-criados/baixo uso. Security Advisor sem `function_search_path_mutable`, sem `anon_security_definer_function_executable` e sem `public_bucket_allows_listing`; restantes classificados como intencionais ou manuais: RPCs `SECURITY DEFINER` para `authenticated`, `system_logs` sem policy, `pg_trgm` em `public` e Auth leaked password protection.
