...
describe('index.html CSP', () => {
  ...
  it('permite a geracao de PDF pelo @react-pdf/renderer no navegador', () => {
    expect(html).toContain("script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' https://js.stripe.com https://www.googletagmanager.com")
    expect(html).toContain("connect-src 'self' data:")
    expect(html).toContain("https://www.google-analytics.com")
